...
def draw_camera_bbox3d_on_img(bboxes3d,
    ...
    out_img = plot_rect3d_on_img(img, num_bbox, imgfov_pts_2d, color, thickness)

    # 3) Place text for each bounding box if labels + class_names are provided
    if labels is not None and class_names is not None:
        ...
        for i in range(num_bbox):
            lbl = int(labels[i])
            # Safely handle out-of-range
            if 0 <= lbl < len(class_names):
                cls_name = class_names[lbl]
            else:
                cls_name = "unknown"

            # pick a text anchor position – e.g. average of the 8 corners
            corners_2d = imgfov_pts_2d[i]  # shape (8,2)
